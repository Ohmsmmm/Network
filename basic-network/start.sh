...
docker exec -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org1.example.com/msp" peer1.org1.example.com peer channel fetch newest ${CHANNEL_NAME}.block --channelID ${CHANNEL_NAME} --orderer orderer.example.com:7050
...
docker exec -e "CORE_PEER_LOCALMSPID=Org3MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org3.example.com/msp" peer0.org3.example.com peer channel fetch newest ${CHANNEL_NAME}.block --channelID ${CHANNEL_NAME} --orderer orderer.example.com:7050
...
docker exec -e "CORE_PEER_LOCALMSPID=Org3MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org3.example.com/msp" peer1.org3.example.com peer channel fetch newest ${CHANNEL_NAME}.block --channelID ${CHANNEL_NAME} --orderer orderer.example.com:7050
